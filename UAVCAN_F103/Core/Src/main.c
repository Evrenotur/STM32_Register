...
void receiveUAVCANMessage(void)
{

	/*
    // CAN mesajını FIFO0'dan al
    if (HAL_CAN_GetRxMessage(&hcan, CAN_RX_FIFO0, &RxHeader, rx_data) == HAL_OK)
    {
     	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, RESET);
        // Gelen mesajın UAVCAN_MESSAGE_SUBJECT_ID ile eşleşip eşleşmediğini kontrol edin
        if (RxHeader.StdId == UAVCAN_MESSAGE_SUBJECT_ID && RxHeader.DLC == 4)  // Veri boyutu kontrolü (4 byte)
        {
            // Gelen veriyi UART üzerinden gösterin
        ...
        {
            // Hatalı mesaj veya farklı ID
            sprintf(uart_buffer, "Yanlış ID veya Hatalı Mesaj!\n");
            HAL_UART_Transmit(&huart2, (uint8_t*)uart_buffer, strlen(uart_buffer), 100);
        }
    }
    else
    {
        // Mesaj alınamadıysa hata mesajı gönderin
       // sprintf(uart_buffer, "CAN Mesajı Alınamadı!\n");
       // HAL_UART_Transmit(&huart2, (uint8_t*)uart_buffer, strlen(uart_buffer), 100);


        // Mesaj alınamadıysa hata mesajı gönderin
        sprintf(uart_buffer, "CAN Mesajı Alınamadı! Hata kodu: %ld\n", HAL_CAN_GetError(&hcan));
        HAL_UART_Transmit(&huart2, (uint8_t*)uart_buffer, strlen(uart_buffer), 100);
    }

    */
	  // CAN mesajını FIFO0'dan al
	   // CANard ile alım işlemini başlatın

	CanardFrame canard_frame;
	CanardRxTransfer canard_rx_transfer;
	CanardRxSubscription canard_subscription;

	// CANard yapılandırması için kullanılacak CanardInstance ve CANard için diğer tanımlamalar
	CanardInstance canard_instance;

	        int8_t result = canardRxAccept(
	            &canard_instance,
	            HAL_GetTick(),    // Zaman damgası
	            &canard_frame,    // Gelen CANard mesaj çerçevesi
	            0,                // Interface index
	            &canard_rx_transfer,    // Alınan mesaj verisi
	            NULL    // Abonelik için kullanılır, şu an NULL
	        );

	        // Eğer mesaj başarıyla alındıysa
	        if (result == 1)
	        {
	            sprintf(uart_buffer, "Mesaj alindi: %02X %02X %02X %02X\n",
	                    rx_data[0], rx_data[1], rx_data[2], rx_data[3]);
	            HAL_UART_Transmit(&huart2, (uint8_t*)uart_buffer, strlen(uart_buffer), 100);
	        }
	        else
	        {
	            // Mesaj alım hatası
	            sprintf(uart_buffer, "Mesaj alimi basarisiz! Hata kodu: %d\n", result);
	            HAL_UART_Transmit(&huart2, (uint8_t*)uart_buffer, strlen(uart_buffer), 100);
	        }
  ...
  //    HAL_CAN_AddTxMessage(&hcan, &pTXHeader, data, &pTxMailbox); //bu iki değeri gönderiyor
	// HAL_CAN_AddTxMessage(&hcan, &pTXHeader2, data, &pTxMailbox);
	 // HAL_CAN_AddTxMessage(&hcan, &pTXHeader3, data, &pTxMailbox);
	  //HAL_CAN_AddTxMessage(&hcan, &pTXHeader10, data, &pTxMailbox);
